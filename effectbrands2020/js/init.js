...
function sethint(hit) {
	...
	if (hit == 1) {
		loadvotecount('25', '100') //28
	} else if (hit == 2) {
		loadvotecount('26', '100') //18
	}
}
...
function mainpage() {
	hint()
	if (
		sessionStorage['l1Contents'] == null ||
		sessionStorage['l2Contents'] == null ||
		(sessionStorage['l1chose'] == null && sessionStorage['l2chose'] == null)
	) {
		getpublicServicelist('100') //28
		getnetServicelist('100') //18
	} else {
		$('#publicServiceList').html(sessionStorage['l1Contents'])
		$('#netServiceList').html(sessionStorage['l2Contents'])
		loadvotecount('25', '100')
		loadvotecount('26', '100')
...
function hint() {
	...
	if (hint == 0) {
		...
		$('#pills-publicService-tab').addClass('active')
		$('#pills-netService-tab').removeClass('active')
		$('#pills-rank-tab').removeClass('active')

		$('#pills-rule').removeClass('show active')
		$('#pills-publicService').addClass('show active')
		$('#pills-netService').removeClass('show active')
		$('#pills-rank').removeClass('show active')
	} else if (hint == 2) {
		$('.fixbutton').css('display', 'block')

		$('#pills-rule-tab').removeClass('active')
		$('#pills-publicService-tab').removeClass('active')
		$('#pills-netService-tab').addClass('active')
		$('#pills-rank-tab').removeClass('active')

		$('#pills-rule').removeClass('show active')
		$('#pills-publicService').removeClass('show active')
		$('#pills-netService').addClass('show active')
		$('#pills-rank').removeClass('show active')
	} else if (hint == 3) {
		$('.fixbutton').css('display', 'none')
...
function backfromNext(list, pro) {
	...
	if (!(sessionStorage[list] == null)) {
		...
		if (str != '') {
			...
			for (var i = 0; i < num; i++) {
				$('#kbchks2020' + str.substring(0, str.length - 1).split(',')[i]).prop(
					'checked',
					true
				)
			}
...
function loadvotecount(listtype, num) {
	...
	$.ajax({
		type: 'get',
		url: link +
			'api/v2/getActivityItemList?activityCode=' +
			actCODE +
			'&activityTagId=' +
			listtype +
			'&count=' +
...
function getpublicServicelist(num) {
	$.ajax({
		type: 'get',
		url: link +
			'api/v2/getActivityItemList?activityCode=' +
			actCODE +
			'&activityTagId=25&count=' +
			num,
		dataType: 'json',
		beforeSend: function () {
			//请求开始至成功显示加载条
		...
		success: function (receiver) {
			//清空列表显示区域
			$('#publicServiceList').html('')
			//隐藏加载条
			$('#l1loading').css('display', 'none')
			console.log(receiver.data)
			for (var i = 0; i < receiver.data.length; i++) {
				var vote0 =
					'<div class="list-group-item"><img class="listImg" src="' +
					imgPath +
					receiver.data[i].imgPath +
					'" alt=""><div class="listNotag">No.' +
					receiver.data[i].id +
					'</div><div class="listMain">'
				var vote1 =
					'<div class="listtitle"><a href="detail.html?id=' +
					receiver.data[i].id +
					'">' +
					receiver.data[i].title +
					'</a></div><div class="listcount">'
				var vote2 =
					iconcc +
					'<b id="l1Vcounts' +
					receiver.data[i].id +
					'">' +
					receiver.data[i].voteCount +
					'</b></div></div><div class="listbtn">'
				var vote3 =
					'<input id="kbchks2020' +
					receiver.data[i].id +
					'" type="checkbox" name="list1chk" class="kbcheck" value="' +
					receiver.data[i].id +
					'" onclick=bindkbChecks("list1chk","list1pro")></div></div>'
				$('#publicServiceList').append(vote0 + vote1 + vote2 + vote3)
			}
			sessionStorage.setItem('l1Contents', $('#publicServiceList').html())
			backfromNext('l1chose', 'list1pro')
			$('.kbmaincoverforloading').fadeOut(fadeOuttime)
		},
		error: function () {
...
function getnetServicelist(num) {
	$.ajax({
		type: 'get',
		url: link +
			'api/v2/getActivityItemList?activityCode=' +
			actCODE +
			'&activityTagId=26&count=' +
			num,
		dataType: 'json',
		beforeSend: function () {
			//请求开始至成功显示加载条
		...
		success: function (receiver) {
			...
			for (var i = 0; i < receiver.data.length; i++) {
				...
				var vote3 =
					'<input id="kbchks2020' +
					receiver.data[i].id +
					'" type="checkbox" name="list2chk" class="kbcheck" value="' +
					receiver.data[i].id +
					'" onclick=bindkbChecks("list2chk","list2pro")></div></div>'
				$('#netServiceList').append(vote0 + vote1 + vote2 + vote3)
				// console.log(receiver.data[i].voteCount)
			}
			sessionStorage.setItem('l2Contents', $('#netServiceList').html())
			backfromNext('l2chose', 'list2pro')
		},
		error: function () {
			console.log(getnetServicelist + '--Error')
			return
		},
	})
}
...
function getrank(ranktype) {
	...
	var rload = ''
	if (ranktype == 25) {
		// ranklist = 'publicServiceRank'
		ranklist = 'netServiceRank'
	...
	$.ajax({
		type: 'get',
		url: link +
			'api/v2/getActivityItemsRanking?activityCode=' +
			actCODE +
			'&activityTagId=' +
			ranktype,
		dataType: 'json',
		...
		success: function (receiver) {
			...
			if (receiver.data.length == 0) {
				...
				for (var i = 0; i < receiver.data.length; i++) {
					...
					if (i == 0) {
						rank0 =
							'<div class="list-group-item"><div class="rankNo"><img src="img/1.png"></div>'
					} else if (i == 1) {
						rank0 =
							'<div class="list-group-item"><div class="rankNo"><img src="img/2.png"></div>'
					} else if (i == 2) {
						rank0 =
							'<div class="list-group-item"><div class="rankNo"><img src="img/3.png"></div>'
					} else {
						rank0 =
							'<div class="list-group-item"><div class="rankNo">' +
							receiver.data[i].rowNo +
							'</div>'
					}
					var rank1 =
						'<div class="rankTitle"><div class="rankline1"><a href="detail.html?id=' +
						receiver.data[i].activityItemId +
						'">' +
						receiver.data[i].title +
						'</a>'
					var rank2 =
						'</div></div><div class="rankCount">' +
						receiver.data[i].count +
						'</div></div>'
...
function initClock() {
	...
	var i = '2020__regexoperators___/12/31 23:59:59'
	...
	var config = {
		...
		onFinish: function () {
			$('#pills-servicecase,#pills-policyservice').html(
				'<div style="margin:20px auto;text-align:center;color:#fff">投票已结束</div>'
			)
...
function isMyhaveprize() {
	$.ajax({
		type: 'get',
		url: 'https://api.foshanplus.com/foshan/api/v2/getMyPrizes?startId=-1&count=10&activityCode=nh_influence_2020&token=' +
			authInfo.fs_token,
		dataType: 'json',
		success: function (data) {
...
function myprizes() {
	$.ajax({
		...
		error: function () {
			console.log('myprizes*****xxx*****获取奖品列表失败')
			return
		},
	})
}
