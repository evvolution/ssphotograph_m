...
var newstitle = ''
var newsdesc = '立即参与投票！P40手机、小米手环、现金红包等你来抽'
var authInfo = {
	appid: 'wx39fac88debc390da',
	api: 'https://api.foshanplus.com/foshan/api/v2',
	activityId: 4, // 活动id
	activityCode: 'nh_influence_2020',
	activity: {
		description: '佛山+红包抽奖',
		title: '佛山+红包抽奖',
	...
	activityLottery: function (successBack, failBack) {
		$.ajax({
			...
			success: (res) => {
				console.log(res)
				// alert('ret:' + res.ret + 'sendStatus:' + res.data.sendStatus + 'prizeName:' + res.data.prizeName + 'amount:' + res.data.amount)
				if(res.ret == 0) {
					$('#red_base').fadeOut(50);//红包封面隐藏
					$('#red_null').fadeIn(50); //未中奖显示
				} else if((res.ret == 1) && (res.data.sendStatus == 0)){
					$('#red_base').fadeOut(50);//红包封面隐藏
					alert('红包已经派发至‘我的奖品’,请分享本活动页面至微信打开即可自动领取');
					$('#red_get_num').html(parseFloat(parseInt(res.data.amount) / 100)+'元红包')
					$('#red_get').fadeIn(50); //中奖但是未领奖，待派奖
				} else if((res.ret == 1) && (res.data.sendStatus == 1)) {
					$('#red_base').fadeOut(50);//红包封面隐藏
					$('#red_get_num').html(parseFloat(parseInt(res.data.amount) / 100)+'元红包')
					$('#red_get').fadeIn(50); //中奖，派奖成功
				}
	...
	$.ajax({
		...
		success: (res) => {
			if (res.ret == 1) {
				...
				res.link = 'https://content.foshanplus.com/h5/effectbrands2020/index.html'
				initSign(res)
			}
		},
	})
