...
var fcup_upload = {
    ...
    fcup_upload: function () {
        ...
        if (!((typex === "mp4") || (typex === "MP4") || (typex === "mov") || (typex === "MOV") || (typex === "mpeg") || (typex === "MPEG"))) {
            swal({
                title: "只允许上传mp4,mpeg或mov格式的文件",
                icon: "error",
            });
            $("#uploadx_input").val("");
        ...
        var fileMD5 = (name + timestamp).toString().MD5();
...
var fcup_upload_pic = {
    fcup_pic: function (config) {
        jQuery.extend(config);
        if (jQuery.upId_pic && jQuery.upUrl_pic) {
            jQuery.domHtml_pic = jQuery('#' + jQuery.upId_pic).html();
            jQuery.fcup_addFileInput_pic();
        }
    },
    fcup_limitFileSize_pic: function (file, limitSize) {
        var arr = ["KB", "MB", "GB"],
            limit = limitSize.toUpperCase(),//toUpperCase方法用于把字符串转换为大写。
            limitNum = 0;
        for (var i = 0; i < arr.length; i++) {
    ...
    upStop_pic: function (err) {
        jQuery.upError_pic = err;
    },
    upStatus_pic: function () {
        if (jQuery.upError) {
            if (typeof jQuery.upStop_pic == 'function') {
                jQuery.upStop_pic(jQuery.upError_pic);
            }
            return false;
        }
        return true;
    },
    fcup_getPercent_pic: function (num, total) {
        num = parseFloat(num);
        total = parseFloat(total);
        if (isNaN(num) || isNaN(total)) {
            return "-";
    ...
    fcup_addFileInput_pic: function () {
        jQuery.upInputId_pic = jQuery.upId_pic + '_input';
        var C = jQuery('#' + jQuery.upId_pic).attr("class");
        var X = jQuery('#' + jQuery.upId_pic).position().top;
        var Y = jQuery('#' + jQuery.upId_pic).position().left;
        var W = jQuery('#' + jQuery.upId_pic).innerWidth();
        var H = jQuery('#' + jQuery.upId_pic).innerHeight();
        var html = jQuery.domHtml_pic;
        if (C) {
            html += '<input type="file" id="' + jQuery.upInputId_pic + '" class="' + C + '" onchange="jQuery.fcup_upload_pic()" style="position:absolute;left:' + Y + 'px;top:' + X + 'px;opacity:0;z-index:9999;width:' + W + 'px;height:' + H + 'px;">';
        } else {
            html += '<input type="file" id="' + jQuery.upInputId_pic + '" onchange="jQuery.fcup_upload_pic()" style="position:absolute;left:' + Y + 'px;top:' + X + 'px;opacity:0;z-index:9999;width:' + W + 'px;height:' + H + 'px;">';
        }
        jQuery('#' + jQuery.upId_pic).html(html);
    },
    fcup_upFileInput_pic: function () {
        var X = jQuery('#' + jQuery.upId_pic).position().top;
        var Y = jQuery('#' + jQuery.upId_pic).position().left;
        var W = jQuery('#' + jQuery.upId_pic).innerWidth();
        var H = jQuery('#' + jQuery.upId_pic).innerHeight();
        var obj = document.getElementById(jQuery.upInputId_pic);
        obj.style.cssText = 'position:absolute;left:' + Y + 'px;top:' + X + 'px;opacity:0;z-index:9999;width:' + W + 'px;height:' + H + 'px;';
    },
    fcup_upload_pic: function () {
        var result = '';
        jQuery.upError_pic = '';
        ...
        console.log(types);
        if (!((types == "jpg") || (types == "JPG") || (types == "jpeg") || (types == "JPEG") || (types == "png") || (types == "PNG"))) {
            swal({
                title: "只允许上传jpg,jpeg或png格式的图片",
                icon: "error",
            });
            $("#uploads_input").val("");
        ...
        var index2 = name.length,
            suffix = name.substring(index1 + 1, index2),
            shardSize = jQuery.upShardSize_pic * 1024 * 1024,
            succeed = 0,
            shardCount = Math.ceil(size / shardSize);

        if (jQuery.upType_pic) {
            uptype = jQuery.upType_pic.split(",");
            if (jQuery.inArray(suffix, uptype) == -1) {
                jQuery.upError_pic = '不允许上传的文件类型-' + suffix;
            }
        }
        if (jQuery.upMaxSize_pic) {
            if (!jQuery.fcup_limitFileSize_pic(file, jQuery.upMaxSize_pic + 'MB')) {
                jQuery.upError_pic = '上传文件过大';
            }
        }
        if (jQuery.upStatus_pic() == false) {
            return false;
        }
        setInterval("jQuery.fcup_upFileInput_pic();",500);
        var re = [];
        var start, end = 0;
        for (var i = 0; i < shardCount; ++i) {
            re[i] = [];
            ...
            if (stack[i2]) {
                ...
                xhr.onreadystatechange = function () {
                    ...
                    if (xhr.readyState == 4 && xhr.status == 200) {
                        var data = xhr.responseText ? eval('(' + xhr.responseText + ')') : '';
                        ++succeed;
                        var cent = jQuery.fcup_getPercent_pic(succeed, shardCount);
                        if (typeof jQuery.upEvent == 'function') {
                            jQuery.upEvent_pic(cent);
                        }
                        if (cent == 100) {
                            setTimeout(function () {
                                if (typeof jQuery.upCallBack == 'function') {
                                    jQuery.upCallBack_pic(data);
                                }
                            }, 500);
                        } else {
                            if (typeof jQuery.upCallBack_pic == 'function') {
                                jQuery.upCallBack_pic(data);
                            }
                        }
                    }
