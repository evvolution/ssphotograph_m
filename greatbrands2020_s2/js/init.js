...
function sethint(hit) {
    ...
    if(hit == 1){
        loadvotecount('18','34') //28
    }else if(hit == 2){
        loadvotecount('19','22') //18
    }
}
...
function mainpage() {
    ...
    if((sessionStorage['l1Contents'] == null) || (sessionStorage['l2Contents'] == null) || ((sessionStorage['l1chose'] == null) && (sessionStorage['l2chose'] == null))) {
        getpublicServicelist('34') //28
        getnetServicelist('22') //18
    } else {
...
function getpublicServicelist(num) {
    $.ajax({
        type: "get",
        url: link + 'api/v2/getActivityItemList?activityCode=' + actCODE + '&activityTagId=18&count=' + num,
        dataType:"json",
        beforeSend: function() {
            //请求开始至成功显示加载条
...
function getnetServicelist(num) {
    $.ajax({
        type: "get",
        url: link + 'api/v2/getActivityItemList?activityCode=' + actCODE + '&activityTagId=19&count=' + num,
        dataType:"json",
        beforeSend: function() {
            //请求开始至成功显示加载条
        ...
        success:function(receiver) {
            ...
            for(var i = 0; i < receiver.data.length; i++) {
                var vote0 = '<div class="list-group-item"><div class="listNotag">No.' + receiver.data[i].id + '</div><div class="listNo">' + receiver.data[i].sno + '</div><div class="listMain">'
                var vote1 = '<div class="listtitle"><a href="detail.html?id=' + receiver.data[i].id + '">' + receiver.data[i].title + '</a></div><div class="listcount">'
                var vote2 = iconcc + '<b id="l2Vcounts' + receiver.data[i].id + '">' + receiver.data[i].voteCount + '</b></div></div><div class="listbtn">'
            ...
            if(receiver.data.length == 0){
                ...
                for(var i = 0; i < receiver.data.length; i++) {
                    ...
                    if(i == 0) {
                        ...
                        rank0 = '<div class="list-group-item"><div class="rankNo">' + receiver.data[i].rowNo + '</div>'
                    }
                    ...
                    var rank2 = '</div></div><div class="rankCount">' + receiver.data[i].count + '</div></div>'
...
function votepush(){
    ...
    $.ajax({
        type:"post",
        url: link + 'api/v2/votesByActivityDayOnlyOne2',
        data:{
        ...
        success:function(data){
            ...
            if(data.ret == 1){
                ...
                if(data.msg == "今天已点赞3次，请明天再点赞！"){
                    ...
                    sessionStorage.setItem('l1chose', '')
                    sessionStorage.setItem('l2chose', '')
                    window.location.href = 'https://content.foshanplus.com/h5/greatbrands2020_s2/index.html'
                }
                ...
                // }
                    ...
                    //异常，去404
                    window.location.href = 'https://content.foshanplus.com/h5/greatbrands2020_s2/404.html'
                }
            }
...
function initClock() {
    ...
    var i = '2020__regexoperators___/12/2 23:59:59';
